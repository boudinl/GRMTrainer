        ...
        <div className="mt-5">
          {Object.keys(groupedProducts).map((type) => (
            <div key={type} className="mb-5">
              ...
              <div className="grid grid-cols-2 gap-6 sm:grid-cols-4 sm:gap-3 lg:mt-8">
                {groupedProducts[type].map((item) => (
                  <article
                    className="bg-black border-white rounded-md relative flex flex-col overflow-hidden border-2 cursor-pointer"
                    key={item._id}
                  >
